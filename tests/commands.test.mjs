...
test("all command files exist", () => {
  const expected = ["setup", "task", "rescue", "review", "adversarial-review", "status", "result", "cancel"];
  for (const name of expected) {
    assert.ok(fs.existsSync(path.join(COMMANDS_DIR, `${name}.md`)), `Missing: ${name}.md`);
  }
});
...
test("adversarial-review.md has disable-model-invocation: true", () => {
  assert.ok(readCommand("adversarial-review").includes("disable-model-invocation: true"));
});
...
test("adversarial-review.md instructs returning output verbatim", () => {
  assert.ok(readCommand("adversarial-review").toLowerCase().includes("verbatim"));
});
